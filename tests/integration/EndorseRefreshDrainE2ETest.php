...
final class EndorseRefreshDrainE2ETest extends TestCase
{
    ...
    private function makeRunner(array $cfg, string $worker = 'w_e2e'): EndorseRefreshDrainRunner
    {
        ...
        $claim = function (int $n) use ($pdo, $worker) {
            ...
            $out = [];
            foreach ($rows as $r) {
                $out[] = ['queue_id' => intval($r['id']), 'behavior' => $r['behavior'], 'scope' => EndorseRefreshRateScope::scope('rapidapi', 'key-A')];
            }
            return $out;
        };

        $fetch = function (array $item, int $attempt) use (&$attemptSeen, &$clock, $cfg) {
        ...
        $apply = function (array $item, array $resp) use ($pdo) {
            if (!empty($resp['ok'])) {
        ...
        $release = function (array $items) use ($pdo) {
            foreach ($items as $it) {
        ...
        $now = function () use (&$clock) { return $clock['t']; };
        $sleep = function ($s) use (&$clock) { $clock['t'] += $s; };

        return new EndorseRefreshDrainRunner(array_merge([
            'claim' => $claim, 'fetch' => $fetch, 'apply' => $apply, 'release' => $release,
            'reserve' => function (string $scope) use ($store, $cfg) {
                return $store->reserve($scope, (int) ($cfg['rate'] ?? 1000), 60, ['run_id' => 'e2e']);
            },
            'now' => $now, 'sleep' => $sleep,
        ], $cfg['runner'] ?? []));
    }
    ...
    public function testMultiChunkDrainCompletesAllSuccesses(): void
    {
        ...
        $completed = intval(self::$pdo->query("SELECT COUNT(*) FROM endorse_refresh_queue WHERE status='completed'")->fetchColumn());
        ...
        $stuck = intval(self::$pdo->query("SELECT COUNT(*) FROM endorse_refresh_queue WHERE status='processing'")->fetchColumn());
