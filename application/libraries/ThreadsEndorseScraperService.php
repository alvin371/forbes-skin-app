...
class ThreadsEndorseScraperService
{
    ...
    protected function submitPending(int $limit): array
    {
        ...
        foreach ($rows as $row) {
            $attemptNo = intval($row['attempt_sequence'] ?? 0) + 1;
            $this->db->insert('endorse_refresh_queue_attempts', [
                'queue_id' => intval($row['id']), 'attempt_no' => $attemptNo,
            ...
            $result = $this->CI->threads_scraper_api->scrapePost(strval($row['link_upload']));
            $data = is_array($result['data'] ?? null) ? $result['data'] : [];
            if (!empty($result['status']) && !empty($data['job_id'])) {
                $this->db->update('endorse_refresh_queue', [
                    'status' => 'submitted', 'provider_job_id' => strval($data['job_id']),
                    'provider_submitted_at' => $now, 'worker_id' => null,
                    'claim_owner' => null, 'started_at' => null, 'claimed_at' => $now,
                ], ['id' => intval($row['id'])]);
                $this->finalizeAttempt(intval($row['id']), $attemptNo, $workerId, 'submitted', null, null, null);
                $summary['submitted']++;
                continue;
            }
            $outcome = $this->retryOrFail($row, $attemptNo, $workerId, strval($result['msg'] ?? 'Gagal membuat job Threads.'), strval($result['error_class'] ?? 'transient'));
            $summary[$outcome]++;
        }
        return $summary;
    }
    ...
    protected function pollSubmitted(int $limit): array
    {
        ...
        $timeout = max(60, intval(env('SOCIAL_SCRAPER_JOB_TIMEOUT_SEC', 900)));
        foreach ($rows as $row) {
            $attemptNo = intval($row['attempt_sequence'] ?? 0);
            $workerId = strval($this->attemptWorkerId(intval($row['id']), $attemptNo));
            $submittedAt = strtotime(strval($row['provider_submitted_at'] ?? '')) ?: time();
            if ((time() - $submittedAt) >= $timeout) {
                $outcome = $this->retryOrFail($row, $attemptNo, $workerId, 'Job Threads melewati batas waktu.', 'transient');
                $summary[$outcome]++;
                continue;
            }
            $job = $this->CI->threads_scraper_api->job(strval($row['provider_job_id']));
            if (empty($job['status'])) {
                // A polling transport error does not create a duplicate remote job.
                $this->db->update('endorse_refresh_queue', ['error_message' => strval($job['msg'] ?? 'Gagal memeriksa job Threads.')], ['id' => intval($row['id'])]);
                $summary['waiting']++;
                continue;
            }
            $remote = $job['data'];
            $status = strval($remote['status'] ?? '');
            if (in_array($status, ['pending', 'running'], true)) {
                $summary['waiting']++;
                continue;
            }
            if ($status !== 'completed') {
                $outcome = $this->retryOrFail($row, $attemptNo, $workerId, strval($remote['error'] ?? 'Job Threads gagal.'), 'transient');
                $summary[$outcome]++;
                continue;
            }

            $response = Threads_scraper_api::normalizePostResult(is_array($remote['result'] ?? null) ? $remote['result'] : [], strval($row['link_upload']));
            ...
            $applied = $purpose === 'daily'
                ...
                : $this->CI->endorse_sync->apply_snapshot($endorse, $response, $purpose, intval($row['enqueued_by'] ?? 0));
            if (empty($applied['status'])) {
                $outcome = $this->retryOrFail($row, $attemptNo, $workerId, strval($applied['msg'] ?? 'Gagal menerapkan statistik Threads.'), strval($applied['error_class'] ?? 'transient'));
                $summary[$outcome]++;
                continue;
            }
            $now = date('Y-m-d H:i:s');
            ...
            $this->finalizeAttempt(intval($row['id']), $attemptNo, $workerId, 'completed', null, null, $now);
            if ($purpose === 'daily') $this->CI->endorse_sync->update_campaign_parent(intval($endorse['id_campaign']));
            $summary['completed']++;
        }
        return $summary;
    }
    ...
    protected function retryOrFail(array $row, int $attemptNo, string $workerId, string $msg, string $errorClass): string
    {
        ...
        $failed = $terminal || $attemptNo >= intval($row['max_attempts']);
        $now = date('Y-m-d H:i:s');
        $this->db->update('endorse_refresh_queue', [
        ...
        $this->finalizeAttempt(intval($row['id']), $attemptNo, $workerId, $failed ? 'failed' : 'retrying', $errorClass, $msg, $now);
    ...
    protected function finalizeAttempt(int $queueId, int $attemptNo, string $workerId, string $status, ?string $class, ?string $msg, ?string $finishedAt): void
    {
        ...
        if ($finishedAt !== null) $data['finished_at'] = $finishedAt;
