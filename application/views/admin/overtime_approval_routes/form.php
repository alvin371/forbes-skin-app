<div class="card" style="border-radius: 2px; border: 1px solid #f0f0f0; box-shadow: 0 2px 8px rgba(0,0,0,0.09);">
    <div class="card-header" style="background-color: #fff; border-bottom: 1px solid #f0f0f0; padding: 16px;">
        <h3 style="margin: 0; font-size: 16px; font-weight: 500; color: rgba(0,0,0,0.85);">
            <?php echo $is_edit ? 'Edit Rute Approval Lembur' : 'Buat Rute Approval Lembur'; ?>
        </h3>
    </div>
    ...
    <div class="card-body" style="padding: 24px;">
        ...
        <form method="post" action="<?php echo $is_edit ? site_url('admin/overtime-approval-routes/' . $route['id'] . '/update') : site_url('admin/overtime-approval-routes/store'); ?>" id="routeForm">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" name="route_code" class="form-control" style="border-radius: 2px; height: 32px; font-size: 14px;"
                                   ...
                                   placeholder="contoh: OVERTIME_ROLE"
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control" style="border-radius: 2px; height: 32px; font-size: 14px;"
                                   ...
                                   placeholder="contoh: Rute Approval Lembur Marketing" required>
                        </div>
                    </div>
                </div>
            ...
            <div style="margin-bottom: 24px; padding-bottom: 24px; border-bottom: 1px solid #f0f0f0;">
                ...
                <small style="display: block; margin-bottom: 12px; color: rgba(0,0,0,0.45);">
                    Tentukan kondisi untuk mencocokkan rute ini dengan pengajuan lembur. Jika tidak ada kondisi, rute ini cocok untuk semua.
                </small>
                    ...
                    <?php if (!empty($route['scopes'])): ?>
                        <?php foreach ($route['scopes'] as $idx => $scope): ?>
                            <div class="scope-row" style="display: flex; gap: 12px; margin-bottom: 12px; padding: 12px; background-color: #fafafa; border-radius: 4px;">
                                ...
                                <select name="scope_value[]" class="form-select" style="flex: 1; border-radius: 2px; height: 32px; font-size: 14px;">
                                    <option value="<?php echo htmlspecialchars($scope['scope_value'] ?? ''); ?>"><?php echo htmlspecialchars($scope['scope_value'] ?? ''); ?></option>
                                </select>
            ...
            <div style="display: flex; gap: 12px; justify-content: flex-end; padding-top: 16px; border-top: 1px solid #f0f0f0;">
                <a href="<?php echo site_url('admin/overtime-approval-routes'); ?>" class="btn btn-secondary" style="border-radius: 2px; height: 32px; padding: 4px 15px; font-size: 14px;">
                    Batal
                </a>
...
function updateScopeValue(select) {
    ...
    var valueSelect = row.querySelector('select[name="scope_value[]"]');
    var scopeType = select.value;

    var html = '<option value="">-- Pilih --</option>';
    if (scopeType === 'user') {
        html += users.map(u => `<option value="${u.id}">${u.full_name} (${u.role_text || '-'})</option>`).join('');
    } else if (scopeType === 'role') {
        html += roles.map(r => `<option value="${r.display_name}">${r.display_name}</option>`).join('');
    }
...
document.querySelectorAll('select[name=\"scope_type[]\"]').forEach(function(select) {
    if (select.value) {
        var row = select.closest('.scope-row');
        var valueSelect = row.querySelector('select[name=\"scope_value[]\"]');
        var savedValue = valueSelect ? valueSelect.value : '';
        updateScopeValue(select);
        valueSelect = row.querySelector('select[name=\"scope_value[]\"]');
        if (valueSelect && savedValue) {
