    ...
    <div class="card-header" style="background-color: #fff; border-bottom: 1px solid #f0f0f0; padding: 16px; display: flex; align-items: center; justify-content: space-between;">
        <div>
            <h3 style="margin: 0; font-size: 16px; font-weight: 500; color: rgba(0,0,0,0.85);">
                <i class="bi bi-inbox"></i> Inbox Approval
            ...
            <small style="color: rgba(0,0,0,0.45);">Pengajuan cuti yang menunggu persetujuan Anda</small>
        </div>
        <div style="display: flex; gap: 8px;">
            <a href="<?php echo site_url('approvals/inbox/history'); ?>" class="btn btn-outline-secondary" style="border-radius: 2px; height: 32px; padding: 4px 15px; font-size: 14px;">
    ...
    <div class="card-body" style="padding: 16px;">
        ...
        <?php if (empty($pending_approvals)): ?>
            <div style="text-align: center; padding: 48px 0;">
                ...
                <p style="margin-top: 16px; color: rgba(0,0,0,0.45); font-size: 14px;">Tidak ada pengajuan yang perlu disetujui saat ini.</p>
            </div>
        <?php else: ?>
                    ...
                    <tbody>
                        <?php foreach ($pending_approvals as $approval): ?>
                            <tr style="border-bottom: 1px solid #f0f0f0;">
                                ...
                                <td style="padding: 12px 8px; font-size: 14px;">
                                    <div style="display: flex; gap: 8px;">
                                        <a href="<?php echo site_url('approvals/inbox/detail/' . $approval['id']); ?>" class="btn btn-outline-primary btn-sm" style="border-radius: 2px; font-size: 12px;">
    ...
    <div class="modal-dialog">
        <div class="modal-content" style="border-radius: 2px;">
            <div class="modal-header" style="border-bottom: 1px solid #f0f0f0; padding: 16px; background-color: #f6ffed;">
                <h5 class="modal-title" style="font-size: 16px; font-weight: 500; color: #52c41a;"><i class="bi bi-check-circle"></i> Setujui Pengajuan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form id="quickApproveForm">
                <div class="modal-body" style="padding: 16px;">
                    <p>Anda akan menyetujui pengajuan cuti dari <strong id="approveRequesterName"></strong>.</p>
    ...
    <div class="modal-dialog">
        <div class="modal-content" style="border-radius: 2px;">
            <div class="modal-header" style="border-bottom: 1px solid #f0f0f0; padding: 16px; background-color: #fff2f0;">
                <h5 class="modal-title" style="font-size: 16px; font-weight: 500; color: #ff4d4f;"><i class="bi bi-x-circle"></i> Tolak Pengajuan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form id="quickRejectForm">
                <div class="modal-body" style="padding: 16px;">
                    <p>Anda akan menolak pengajuan cuti dari <strong id="rejectRequesterName"></strong>.</p>
...
function submitApprove() {
    ...
    fetch('<?php echo site_url('approvals/inbox/quick-approve'); ?>', {
        method: 'POST',
        headers: {
...
function submitReject() {
    ...
    fetch('<?php echo site_url('approvals/inbox/quick-reject'); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'X-Requested-With': 'XMLHttpRequest'
